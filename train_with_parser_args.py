...
datadict_train = BratPro_Reader(data['train_dicts'][:],transform=train_transforms)
datadict_val = BratPro_Reader(data['val_dicts'][:],transform=val_transforms)
...
model = monai.networks.nets.resnet50(pretrained = False,n_input_channels = 3, num_classes =4).to(device)

model_name = 'ResNet' +"/{:%d_%m_%y}/".format(datetime.now())
