...
trainloader = DataLoader(datadict_train, batch_size=4, shuffle=True)
...
model1 = monai.networks.nets.resnet50(pretrained = False,n_input_channels = 6, num_classes =4).to(device)
model2 = monai.networks.nets.UNet(spatial_dims=3, in_channels = 3,out_channels=3,channels =[16,32,64,128],strides=[2,2,2]).to(device)
...
for epoch in range(0,num_epochs):
    ...
    with tqdm(range(len(trainloader))) as pbar:
        for i, data in zip(pbar, trainloader):
            ...
            input_model1 = torch.cat([output,image],axis=1)
            output2 = model1.forward(input_model1)
